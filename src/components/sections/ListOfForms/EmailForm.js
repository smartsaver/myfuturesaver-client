...
import surveyFormsContent from '../../../content/surveyFormsContent'
...
class MailChimpForm extends Component {
  ...
  render() {
    return <div className="js-mailChimpForm" />
  }
}

const EmailForm = () => {
  const { email } = surveyFormsContent
  return (
    <Section className="Section--blue" name="email-survey">
      <h2 className="title">Enter Email</h2>
      <div
        className="content"
        dangerouslySetInnerHTML={{ __html: email.intro }}
      />
      <MailChimpForm />
    </Section>
  )
